...
Builder handoffs are bounded to 3,600,000 ms by default because discovery and PDF import can be slow; reviewer handoffs remain bounded to 600,000 ms. The legacy `handoffTimeoutMs` option still overrides both limits, while `builderHandoffTimeoutMs` and `reviewerHandoffTimeoutMs` configure them independently. Before a builder starts, the tool lists reusable full-text papers already imported into the run. If any are available, discovery and import tools are removed from that child call so it reads one existing paper instead of downloading another copy. Otherwise, discovery receives bounded structured search, web search, and import access so a blocked index PDF can be replaced by a public direct PDF for the same candidate.

## Understand the implementation

`src/index.ts` owns all eleven schemas and maps runtime and domain failures to root-cause, safe-retry, and stop-condition guidance. Evidence-policy and edge-semantic failures have dedicated stable codes. Builder and reviewer output schemas are closed, their child depth is one, and their tool allowlists are separately bounded. Builders must verify every proposed literal quote before returning; reviewers repeat that verification independently. Evidence verification resolves PDF whitespace and typography variants to `canonical_evidence_text`, but final tree validation still requires that exact returned substring. A deterministic evidence audit converts model `accept` decisions into `revise` when quotes are fragments or omit claimed numeric values. Finalization exports the file contract, while `supramas_artifacts_sync` repairs an interrupted export from durable state. Every workflow mutation requires the exact revision returned by the preceding call. The `direct` mode exists only for trusted integration callers and tests; do not expose it in a model-facing preset.

An unsuccessful atomic handoff never increments the workflow revision or attempt counters. Timeout, caller cancellation, and independent child aborts have distinct error codes. After the first failure, the exact run, revision, and role may be retried once in the same process. A second failure latches that handoff key so additional calls cannot start another expensive child; restart the process before resuming that exact revision. Concurrent duplicate calls are rejected while one handoff is active.
